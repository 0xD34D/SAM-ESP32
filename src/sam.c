...
void Init() {
  ...
  phonemeindex[255] = END;  // to prevent buffer overflow // ML : changed from
...
void Parser2() {
  ...
  while ((p = phonemeindex[pos]) != END) {
    ...
    if ((pf & FLAG_DIPTHONG))
      ...
      // EXAMPLE: AWAY EIGHT
      if (!phonemeindex[pos +
                        1]) {  // If following phoneme is a pause, get next
        p = phonemeindex[pos + 2];
        if (p != END && (flags[p] & FLAG_VOWEL) && stress[pos + 2]) {
          drule(
              "Insert glottal stop between two stressed vowels with space "
              "between them");
...
void AdjustLengths() {
  ...
  // loop index
  {
    ...
    while ((index = phonemeindex[X]) != END) {
      ...
      while (--X && !(flags[phonemeindex[X]] &
                      FLAG_VOWEL));  // back up while not a vowel
      if (X == 0) break;
      ...
      do {
        ...
        if (!(flags[index] & FLAG_FRICATIVE) ||
          ...
          drule_pre(
              "Lengthen <FRICATIVE> or <VOICED> between <VOWEL> and "
              "<PUNCTUATION> by 1.5",
              X);
    ...
    if (flags[index] & FLAG_VOWEL) {
      ...
      if (!(flags[index] & FLAG_CONSONANT)) {
        if ((index == 18) || (index == 19)) {  // 'RX', 'LX'
          ...
          if ((flags[index] & FLAG_CONSONANT)) {
            drule_pre(
                "<VOWEL> <RX | LX> <CONSONANT> - decrease length of vowel by "
                "1\n",
                loopIndex);
      ...
      if (index != END && (flags[index] & FLAG_STOPCONS)) {
        ...
        drule(
            "<UNVOICED STOP CONSONANT> {optional silence} <STOP CONSONANT> - "
